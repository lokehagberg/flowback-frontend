...
>
	<div>
		...
		<p class="pl-6 pr-6 pb-6 break-words">
			{$_('Number')}: <code>{account.account_number}</code><br />
			{$_('Balance')}: {account.balance}
			<br />
			<a href="/accounts/{account.id}/transactions">Transactions</a>
		</p>
		<Button action={update}>{$_('Update')}</Button>
		<Button action={() => (consideringRemoving = true)} buttonStyle="warning">Remove</Button>
		<br />&nbsp;
	</div>
</div>
<AccountRemovalModal bind:open={consideringRemoving} {account} />
