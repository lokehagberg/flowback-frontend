<script lang="ts">
	...
	export let newOne: boolean, id: number;
	let name: string = newOne ? '' : '...',
	...
	onMount(async () => {
		...
		const account = accounts.filter((x: any) => +x.id === +id)[0];
...
<Loader bind:loading>
	<form
		...
		"
	>
		<div class="bg-white p-6 shadow-xl flex flex-col gap-6 md:w-2/5 dark:bg-darkobject">
			<h1 class="text-2xl dark:text-white">{newOne ? 'Add an' : 'Update'} Account</h1>
