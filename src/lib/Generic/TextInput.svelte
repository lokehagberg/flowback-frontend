	...
	{#if type === 'text'}
		<div class="relative w-full">
			...
			<input
				bind:value
				id={`textinput-${label}`}
				class={`${search && "text-right"} dark:text-darkmodeText dark:bg-darkbackground border border-gray-300 border-solid rounded focus:bg-gray-100 p-0.5 w-full outline-none ${inputClass} ${search ? ' pl-10 ' : ''}`}
				{required}
				maxlength={max}
