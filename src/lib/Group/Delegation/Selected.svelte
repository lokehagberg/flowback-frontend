<script lang="ts">
	...
	const saveDelegation = async () => {
		...
		delegates.forEach((delegate) => {
			...
			else if (lastSavedRelation?.tags.length === 0 && delegateTagIds.length > 0)
				createDelegateRelation(delegate.pool_id, delegateTagIds);
			else if (lastSavedRelation && lastSavedRelation?.tags.length > 0)
				updateDelegateRelation(delegate.pool_id, delegateTagIds);
		});
	};
	...
	const updateDelegateRelation = async (delegate_pool_id: number, tags: number[]) => {
