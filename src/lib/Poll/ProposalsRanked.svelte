<script lang="ts">
	import Proposal from '$lib/Poll/Proposal.svelte';
	import { onMount } from 'svelte';

	interface Proposal {
		title: string;
		description: string;
		id: number;
	}

	let proposals: Proposal[] = [
		{
			title: 'Nuclear Option',
			description: `Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quia repudiandae nesciunt ipsam quo
			earum, nemo error necessitatibus minima eligendi voluptatem vel? Praesentium dicta reiciendis
			ut modi id iure minus amet.`,
			id: 1
		},
		{
			title: 'Geothermal Power explosion',
			description: `Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quia repudiandae nesciunt ipsam quo
			earum, nemo error necessitatibus minima eligendi voluptatem vel? Praesentium dicta reiciendis
			ut modi id iure minus amet.`,
			id: 2
		},
		{
			title: 'Evil Pollen',
			description: `Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quia repudiandae nesciunt ipsam quo
			earum, nemo error necessitatibus minima eligendi voluptatem vel? Praesentium dicta reiciendis
			ut modi id iure minus amet.`,
			id: 3
		}
	];

	let ranked: Proposal[] = [];
	let abstained: Proposal[] = proposals;
	let rankedId: number[] = [];
	
	//Removes the proposal from its current container (be it abstain or rank) and move it to the other one
	//Potential speedup can be done by using a dictionary with id's and rank placement
	const switchProposalContainer = (proposalId1: number) => {
		const rankedIdIndex = rankedId.findIndex((proposal) => proposal === proposalId1);
		if (rankedIdIndex === -1) rankedId.push(proposalId1);
		else rankedId.splice(rankedIdIndex, 1);

		ranked = proposals.filter((proposal) => rankedId.includes(proposal.id));
		abstained = proposals.filter((proposal) => !rankedId.includes(proposal.id));
	};

	const swapProposalRanking = (proposalId1: number, proposalId2: number) => {
		const proposal1rank = rankedId.findIndex((proposalId) => proposalId === proposalId1);
		const proposal2rank = rankedId.findIndex((proposalId) => proposalId === proposalId2);

		const b = rankedId[proposal1rank];
		rankedId[proposal1rank] = rankedId[proposal2rank];
		rankedId[proposal2rank] = b;
		rankedId = rankedId;

		// let newRanking = new Array(rankedId.length)
		// ranked = proposals.filter((proposal) => rankedId.includes(proposal.id)).sort(proposal => rankedId)
	};

	onMount(() => {
		setTimeout(() => {
			switchProposalContainer(1);
			switchProposalContainer(2);
			setTimeout(() => {
				swapProposalRanking(1, 2);
			}, 700);
		}, 700);
	});
</script>

<div class="poll border border-gray-700">
		<div style="grid-column: 1 / 2; grid-row: 1 / 2">Abstain</div>
		<div style="grid-column: 2 / 3; grid-row: 1 / 2">Rank</div>
		{#each abstained as proposal, i}
			<div style={`grid-column: 1 / 2; grid-row:${i + 2} / ${i + 2 + 1}`}>
				<Proposal title={proposal.title} description={proposal.description} />
			</div>
		{/each}
		{#each ranked as proposal, i}
			<div style={`grid-column: 2 / 3; grid-row:${i + 2} / ${i + 2 + 1}`}>
				<Proposal title={proposal.title} description={proposal.description} />
			</div>
		{/each}
		{#each ranked as num, i}
			<div style={`grid-column: 3 / 4; grid-row: ${i + 2} / ${i + 2 + 1}; text-align:center`}>
				{i + 1}
			</div>
		{/each}
	</div>

<style>
	.poll {
		display: grid;
		grid-template-columns: 8fr 8fr 1fr;

		align-items: center;
	}
</style>

